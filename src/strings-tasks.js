...
function formatTime(minutes, seconds) {
  ...
  // return `${min}:${sec}`;
  const min = minutes <= 9 ? `${minutes}`.padStart(2, '0') : minutes;
  const sec = seconds <= 9 ? `${seconds}`.padStart(2, '0') : seconds;
  return `${min}:${sec}`;
}
...
function isPalindrome(str) {
  const strReplaced = str.toUpperCase().replace(/[^A-Za-z0-9]/g, '');
  return strReplaced.split('').reverse().join('') === strReplaced;
}
